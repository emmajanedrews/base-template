import os
filedir = "_posts/"
dirs = os.listdir(filedir)
# from bs4 import BeautifulSoup
# soup = BeautifulSoup('_posts/2017-03-06-how-to-maximize-your-homes-sale-price.md', 'html.parser')

for postfile in dirs:
	post = '_posts/' + postfile

	def addFrontMatter():
		post = '_posts/' + postfile
		file = open(post, 'r+')
		yt = file.read()
		ytcode = yt.partition('www.youtube.com/embed/')[2].partition('"')[0]
		newline = yt.replace('---', '---\nyoutube_code: '+ ytcode + '\nexcerpt:\nenclosure:\npullquote:\nenclosure_type: video/mp4\nenclosure_time:\nuse_youtube_image: true\nyoutube_alternate_image: /img/cover.jpg', 1)
		file.seek(0)
		file.write(newline)
		file.close()

	def cleanPost():
		post = '_posts/' + postfile
		file = open(post, 'r+')
		yt = file.read()
		frontmatter = yt.rpartition('---')[0]
		post = yt.partition('</iframe>')[2]
		ytpost = "---\n{% include youtube.html %}\n" + post
		fullpost = (frontmatter + ytpost)
		file.seek(0)
		file.write(fullpost)
		file.close()

	def renamePost():
		if post.endswith('.html'):
			os.rename(post, post.replace('.html', '.md'))

	if post.endswith('.html') and post != '_posts/_defaults.md':
		addFrontMatter()
		cleanPost()`
		renamePost()
		print('Post Setup Complete for ' + post)
		# soup = BeautifulSoup(open('_posts/2017-03-06-how-to-maximize-your-homes-sale-price.md'))
		# print(soup.replace('<br />', '<hello>'))
		# # print(newsoup)
